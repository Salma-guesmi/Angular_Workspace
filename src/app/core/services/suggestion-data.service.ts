...
export class SuggestionDataService {
  private readonly _suggestions: Suggestion[] = [
    {
      id: 1,
  ...
  addSuggestion(data: Omit<Suggestion, 'id'>): Suggestion {
    const nextId = this._suggestions.reduce((m, s) => Math.max(m, s.id), 0) + 1;
    const suggestion: Suggestion = { id: nextId, ...data };
